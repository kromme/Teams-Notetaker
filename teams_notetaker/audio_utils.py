...
def extract_audio(
    video_path: str = None, audio_path: str = None, overwrite: bool = True
):
    ...
    if os.path.isfile(audio_path) and overwrite:
        ...
        logger.info(f"File already exists: not overwriting {audio_path}")
        return
    ...
    try:
        ...
        command = (
            f"""ffmpeg {overwrite_param} -i "{video_path}" -ac 1 "{audio_path}" """
        )
        subprocess.call(command, shell=True)
    except Exception as e:
        logger.error("Could not extract audio file from video.", exc_info=e)

    # check whether file is saved
    assert os.path.isfile(audio_path), "Something went wrong saving the audio file"
...
def remove_silences_from_audio(audio_path: str = None) -> str:
    ...
    silenced_audio_path = audio_path.replace(".wav", "_silence_removed.wav")
